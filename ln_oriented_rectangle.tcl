...
proc linked_carb.oriented.rectangle {p6 size geom_center geom_center_att} {
	...
	# The longest side of the rectangle is perpendicular to the plane of the ring, so it shouldn't clash with connecting residues.
	...
	set rectangle_size [expr $size*1]
        set half_length [expr $rectangle_size/(1.2)]
        set thickness [expr $rectangle_size/(1.8)]
...
#	draw sphere $x2 radius 0.2
	...
	set outer_4 [vecadd $t4 $geom_center]
                ...
                # The following function makes the rectangle 3D by creating two points at the geometric center that are perpendicular to the plane of the square (and parallel with the plane of the ring).
...
# Draw the rectangle
	# Front
        ...
        draw triangle $front_1 $front_4 $center_1
	...
	# Connect
        ...
        draw triangle $back_1 $front_4 $front_1
